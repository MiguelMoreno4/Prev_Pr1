...
public class VentanaPrincipal extends JFrame {
    ...
    public VentanaPrincipal() {
        ...
        pnlParams.setBorder(
                BorderFactory.createTitledBorder(
                        null, "Configuración del Algoritmo",
        ...
        spinPob = new JSpinner(new SpinnerNumberModel(100, 1, 99999, 10));
        spinPob.setPreferredSize(dimSpin);

        spinGens = new JSpinner(new SpinnerNumberModel(200, 1, 99999, 50));
        ...
        spinElite = new JSpinner(new SpinnerNumberModel(5, 0, 100, 1));
        ((JSpinner.DefaultEditor) spinElite.getEditor()).getTextField().setEditable(true);
        ...
        pnlParams.add(new JLabel("Cruce:")); pnlParams.add(comboCruceOp);
        ...
        textPaneReporteFinal.setFont(new Font("Monospaced", Font.PLAIN, 12));

        JScrollPane scrollReporte = new JScrollPane(textPaneReporteFinal);
        scrollReporte.setBounds(555, 140, 555, 270);
        scrollReporte.setBorder(BorderFactory.createTitledBorder("Resultado Final MTSP"));
        ...
        scrollCrom.setBounds(20, 530, 520, 80);
    ...
    private void ejecutarAG(ActionEvent e) {
        ...
        new Thread(() -> {
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                textPaneReporteFinal.setText("Ejecutando algoritmo...\n");
            });
            ...
            System.out.println("Base drones: (" + agMTSP.getFlota().get(0).getBaseX() + "," + agMTSP.getFlota().get(0).getBaseY() + ")");
            ...
            if (mtsp == null) {
                SwingUtilities.invokeLater(() ->
                    textPaneReporteFinal.setText("ERROR: A* no encontró rutas válidas.\n")
                );
            ...
            SwingUtilities.invokeLater(() ->
                mostrarResultadoFinalMTSP(mtsp.fitness, seed, agMTSP, tiemposFinales, rutasFinales)
            );
    ...
    private void cargarEscenario(int idx) {
        ...
        textPaneReporteFinal.setText("Escenario: " + nombreEsc + "\n");
    }
    ...
    private void mostrarRutasRealesMTSP(
        ...
        SwingUtilities.invokeLater(() -> mostrarRutasTexto(rutas, puntosControl));
        ...
        for (int d = 0; d < rutas.size(); d++) {
            ...
            for (int idCam : ruta) {
                ...
                List<Point> camino = aStar.calcularRuta(xActual, yActual, destino.x, destino.y, posicionesCamaras);
                if (camino != null) caminoCompleto.addAll(camino);
                xActual = destino.x;
                yActual = destino.y;
            }

            // vuelta a base
            List<Point> vuelta = aStar.calcularRuta(xActual, yActual, dron.getBaseX(), dron.getBaseY(), posicionesCamaras);
            if (vuelta != null) caminoCompleto.addAll(vuelta);

            rutasVisual.add(caminoCompleto);
        }

        // Mostrar cámaras numeradas por orden de visita con color del dron
        SwingUtilities.invokeLater(() -> panelMapa.setCamaras(puntosControl, rutas));
    ...
    private void mostrarCromosomaColoreado(IndividuoMTSP ind, int numDrones, int numCamaras) {
        ...
        Color[] colores = {Color.BLUE, Color.MAGENTA, Color.GREEN, Color.ORANGE, Color.CYAN};
    ...
    private void mostrarResultadoFinalMTSP(double makespan, long semilla, AlgoritmoGeneticoMTSP agMTSP,
                                            List<Double> tiemposPorDron, List<List<Integer>> rutas) {
        ...
        StyleConstants.setFontSize(styleBase, 13);
        ...
        List<Dron> flota = agMTSP.getFlota();
        ...
        try {
            ...
            // SEMILLA
            doc.insertString(doc.getLength(), "SEMILLA: " + semilla + "\n", styleBase);

            // TIEMPOS POR DRON
            doc.insertString(doc.getLength(), "TIEMPOS POR DRON: ", styleBase);
            for (int i = 0; i < tiemposPorDron.size(); i++) {
                Style styleColor = textPaneReporteFinal.addStyle("cT" + i, null);
                StyleConstants.setFontFamily(styleColor, "Monospaced");
                StyleConstants.setFontSize(styleColor, 13);
                StyleConstants.setBold(styleColor, true);
                StyleConstants.setForeground(styleColor, textColors[i % textColors.length]);
                double vel = (i < flota.size()) ? flota.get(i).getVelocidad() : 1.0;
                doc.insertString(doc.getLength(),
                    String.format("D%d(x%.1f): %.1fs  ", i + 1, vel, tiemposPorDron.get(i)),
                    styleColor);
            }
            doc.insertString(doc.getLength(), "\n", styleBase);
            ...
            for (int d = 0; d < rutas.size(); d++) {
                ...
                for (Integer gen : rutas.get(d)) {
                    doc.insertString(doc.getLength(), (gen + 1) + " ", styleColor);
                }
                if (d < rutas.size() - 1) {
